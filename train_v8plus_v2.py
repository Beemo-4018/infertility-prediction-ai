...
"""
====================================================
난임 환자 임신 성공 여부 예측 - v8plus_v2
평가 지표  : ROC-AUC

[v8plus_v2 변경사항]
  CPU 기준 중요도 0인 피처 11개 제거
  - 불임 원인 - 자궁경부 문제
  - 착상 전 유전 검사 사용 여부
  - 불임 원인 - 여성 요인
  - PGD 시술 여부
...
def preprocess(train, test):
    ...
    for df in [train, test]:
        ...
        df['시술_ICSI']      = df['특정 시술 유형'].astype(str).str.contains('ICSI', na=False).astype(int)

        # ── [v8plus_v2] IVF시술_여부 제거 (중요도 0) ──────────
        # df['IVF시술_여부'] = (df['시술 유형'] == 'IVF').astype(int)  ← 제거
        ...
        # 불임 원인 (중요도 0인 세부 항목 제거)
        male_cause_cols = ['불임 원인 - 남성 요인']  # 정자 세부 항목 전부 제거
        female_cause_cols = [
            '불임 원인 - 난관 질환', '불임 원인 - 배란 장애', '불임 원인 - 자궁내막증'
            # 불임 원인 - 여성 요인, 자궁경부 문제 제거
        ]
        ...
        # 결측 여부 (중요도 0인 것 제거)
        # 착상 전 유전 검사 사용 여부_결측 → 유지 (58.0으로 중요도 있음)
        # PGD/PGS_결측 → 유지 (중요도 있음)
        ...
        sperm_issues = df[['불임 원인 - 남성 요인']].sum(axis=1)
        df['정자문제_파트너정자'] = (
    ...
    # ── [v8plus_v2] drop_cols에 중요도 0 피처 명시적 추가 ────
    zero_importance_drops = [
        '불임 원인 - 자궁경부 문제',
        '착상 전 유전 검사 사용 여부',
        '불임 원인 - 여성 요인',
        'PGD 시술 여부',
    ...
    print(f'  피처 수      : {len(feature_cols)}  (v8plus 150개 → v8plus_v2 {len(feature_cols)}개)')
...
def ensemble_and_save(y_train, sub, oof_lgb, oof_xgb, oof_cat,
    ...
    results[f'성능가중']                   = (roc_auc_score(y_train, w_perf[0]*oof_lgb+w_perf[1]*oof_xgb+w_perf[2]*oof_cat), w_perf[0]*pred_lgb+w_perf[1]*pred_xgb+w_perf[2]*pred_cat)
    ...
    # v8plus CPU 비교
    print(f'\n  [참고] v8plus CPU: 0.74071')
    print(f'  [v8plus_v2]:      {best_cv:.5f}  {"🔺 UP" if best_cv > 0.74071 else "🔻 DOWN"}')
