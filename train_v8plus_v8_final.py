...
"""
====================================================
난임 환자 임신 성공 여부 예측 - v8plus_v5
평가 지표  : ROC-AUC

[v8plus_v5 변경사항 - v8plus_v3 대비]
  파생변수 3개 추가 (v5b에서 효과 확인):
  - 난자정자출처조합_te  (난자출처+정자출처 조합 TE)
  - 총배아대비_해동비율  (해동배아 / 총생성배아)
  - IVF_실패부담        (IVF시술 - IVF출산)

  v8plus_v3: 127개 → v8plus_v5: 130개 (예상)
====================================================
"""
...
USE_OPTUNA    = True
OPTUNA_TRIALS = 50
VERSION       = 'v8plus_v8_final'
TE_SMOOTH_COMBO = 50

V2_DROPS = [
    '불임 원인 - 자궁경부 문제', '착상 전 유전 검사 사용 여부',
    '불임 원인 - 여성 요인', 'PGD 시술 여부',
    '불임 원인 - 정자 운동성', '불임 원인 - 정자 형태',
    '불임 원인 - 정자 면역학적 요인', '난자 채취 경과일',
    'PGS 시술 여부', '난자 해동 경과일',
]
V3_DROPS = [
    '불임 원인 - 정자 농도', '신선난자_저장수', '신선난자_저장됨',
    '동결배아_시술', '저장된 신선 난자 수', '배아 해동 경과일',
    '시술 유형', '신선 배아 사용 여부', '정자기증자_있음',
    '배아 해동 경과일_결측', '대리모_여부_f', '시술_ICSI',
]
...
def preprocess(train, test):
    ...
    for df in [train, test]:
        ...
        df['난자기증자_젊음']      = (
            (df['난자기증자_나이_수치'] > 0) & (df['난자기증자_나이_수치'] <= 30)
        ).astype(int)
    ...
    for df in [train, test]:
        ...
        female_cause_cols = ['불임 원인 - 난관 질환', '불임 원인 - 배란 장애', '불임 원인 - 자궁내막증']
        all_cause_cols    = male_cause_cols + female_cause_cols + [
            '남성 주 불임 원인', '남성 부 불임 원인', '여성 주 불임 원인', '여성 부 불임 원인',
            '부부 주 불임 원인', '부부 부 불임 원인', '불명확 불임 원인'
        ]
        df['남성_불임원인_수'] = df[[c for c in male_cause_cols   if c in df.columns]].sum(axis=1)
        df['여성_불임원인_수'] = df[[c for c in female_cause_cols if c in df.columns]].sum(axis=1)
        df['총_불임원인_수']   = df[[c for c in all_cause_cols    if c in df.columns]].sum(axis=1)
        ...
        df['남성요인_ICSI매칭'] = ((df['불임 원인 - 남성 요인'] == 1) & (df['특정 시술 유형'].astype(str).str.contains('ICSI'))).astype(int)
        df['배란장애_자극매칭'] = ((df['불임 원인 - 배란 장애'] == 1) & (df['배란 자극 여부'] == 1)).astype(int)
        df['고령_동결배아조합'] = ((df['고령_여부'] == 1) & (df['해동된 배아 수'].fillna(0) > 0)).astype(int)
        df['정자문제_파트너정자'] = ((df['불임 원인 - 남성 요인'] > 0) & (df['파트너정자_비율'] > 0.5)).astype(int)
        ...
        # ── [V5 신규] 파생변수 3개 ───────────────────────────
        df['총배아대비_해동비율'] = df['해동된 배아 수'] / (df['총 생성 배아 수'] + 1e-6)
        df['IVF_실패부담']        = (df['IVF 시술 횟수_num'] - df['IVF 출산 횟수_num']).clip(lower=0)
        df['난자정자출처조합']    = df['난자 출처'].astype(str) + '_' + df['정자 출처'].astype(str)

    drop_cols = ['ID', TARGET] + count_cols + \
                ['클리닉_나이조합', '클리닉_시술유형조합'] + ALL_DROPS
    ...
    print(f'  피처 수      : {len(feature_cols)}  (v8plus_v3 127 → v8plus_v5 {len(feature_cols)})')
    print(f'  X_train      : {X_train.shape}')
    print(f'  결측치 합계  : {X_train.isnull().sum().sum()}')
    return X_train, y_train, X_test, feature_cols


def add_v6e_features(X_train, y_train, X_test):
    X_train = X_train.copy()
    X_test = X_test.copy()

    combo_col = '시술시기코드_난자정자조합'
    X_train[combo_col] = (
        X_train['시술 시기 코드'].astype(str) + '_'
        + X_train['난자 출처'].astype(str) + '_'
        + X_train['정자 출처'].astype(str)
    )
    X_test[combo_col] = (
        X_test['시술 시기 코드'].astype(str) + '_'
        + X_test['난자 출처'].astype(str) + '_'
        + X_test['정자 출처'].astype(str)
    )

    tr_enc, te_enc = target_encode(
        pd.concat([X_train[[combo_col]], y_train], axis=1),
        X_test[[combo_col]].copy(),
        combo_col,
        TARGET,
        smooth=TE_SMOOTH_COMBO,
    )
    X_train[combo_col + '_te'] = tr_enc
    X_test[combo_col + '_te'] = te_enc

    X_train = X_train.drop(columns=[combo_col])
    X_test = X_test.drop(columns=[combo_col])

    feature_cols = list(X_train.columns)
    print("  v6e 추가 실험  : ['시술시기코드_난자정자조합_te']")
    print(f'  추가 피처 반영 후: {len(feature_cols)}개')
    return X_train, y_train, X_test


def add_v7b_features(X_train, X_test):
    X_train = X_train.copy()
    X_test = X_test.copy()

    X_train['고령_x_단일배아이식'] = X_train['고령_여부'] * X_train['단일 배아 이식 여부']
    X_test['고령_x_단일배아이식'] = X_test['고령_여부'] * X_test['단일 배아 이식 여부']

    feature_cols = list(X_train.columns)
    print("  v7b 추가 실험  : ['고령_x_단일배아이식']")
    print(f'  추가 피처 반영 후: {len(feature_cols)}개')
    return X_train, X_test


def add_v8_features(X_train, y_train, X_test):
    X_train = X_train.copy()
    X_test = X_test.copy()

    combo_col = '난자출처_시술당시나이'
    X_train[combo_col] = X_train['난자 출처'].astype(str) + '_' + X_train['시술 당시 나이'].astype(str)
    X_test[combo_col] = X_test['난자 출처'].astype(str) + '_' + X_test['시술 당시 나이'].astype(str)
    ...
    feature_cols = list(X_train.columns)
    print("  v8 추가 실험   : ['난자출처_시술당시나이_te']")
    print(f'  추가 피처 반영 후: {len(feature_cols)}개')
    return X_train, y_train, X_test


def build_v8_final_features(train, test):
    X_train, y_train, X_test, _ = preprocess(train, test)
    X_train, y_train, X_test = add_v6e_features(X_train, y_train, X_test)
    X_train, X_test = add_v7b_features(X_train, X_test)
    X_train, y_train, X_test = add_v8_features(X_train, y_train, X_test)
    feature_cols = list(X_train.columns)
    return X_train, y_train, X_test, feature_cols
...
def train_lgb(X_train, y_train, X_test, feature_cols, best_params=None):
    ...
    params = {**best_params, 'objective': 'binary', 'metric': 'auc',
              'verbose': -1, 'random_state': SEED, 'n_jobs': -1} if best_params else {
    ...
    for fold, (tr_idx, val_idx) in enumerate(skf.split(X_train, y_train)):
        ...
        print(f'  Fold {fold+1}  AUC: {roc_auc_score(y_val, oof[val_idx]):.5f}  (best_iter: {m.best_iteration_})')
    cv = roc_auc_score(y_train, oof)
    print(f'  LightGBM CV AUC : {cv:.5f}')
    importance = pd.DataFrame({
        'feature': feature_cols, 'importance': np.mean(importance_list, axis=0)
...
def train_xgb(X_train, y_train, X_test):
    ...
    for fold, (tr_idx, val_idx) in enumerate(skf.split(X_train, y_train)):
        ...
        oof[val_idx]  = m.predict_proba(X_val)[:, 1]
        pred         += m.predict_proba(X_test)[:, 1] / N_FOLDS
        print(f'  Fold {fold+1}  AUC: {roc_auc_score(y_val, oof[val_idx]):.5f}  (best_iter: {m.best_iteration})')
    cv = roc_auc_score(y_train, oof)
...
def ensemble_and_save(y_train, sub, oof_lgb, oof_xgb, oof_cat,
                      pred_lgb, pred_xgb, pred_cat, cv_lgb, cv_xgb, cv_cat):
    print('\n' + '=' * 55)
    print('[5] 앙상블 비교 & 저장')
    print('=' * 55)
    print(f'\n  단일 모델 CV AUC:')
    ...
    results = {
        '단순평균':                     (roc_auc_score(y_train, (oof_lgb+oof_xgb+oof_cat)/3),                          (pred_lgb+pred_xgb+pred_cat)/3),
        '성능가중':                     (roc_auc_score(y_train, w_perf[0]*oof_lgb+w_perf[1]*oof_xgb+w_perf[2]*oof_cat), w_perf[0]*pred_lgb+w_perf[1]*pred_xgb+w_perf[2]*pred_cat),
        'LGB강조(0.5/0.25/0.25)':      (roc_auc_score(y_train, 0.5*oof_lgb+0.25*oof_xgb+0.25*oof_cat),                0.5*pred_lgb+0.25*pred_xgb+0.25*pred_cat),
        'LGB+CAT(0.6/0.4)':            (roc_auc_score(y_train, 0.6*oof_lgb+0.4*oof_cat),                               0.6*pred_lgb+0.4*pred_cat),
        'LGB+CAT(0.5/0.5)':            (roc_auc_score(y_train, 0.5*oof_lgb+0.5*oof_cat),                               0.5*pred_lgb+0.5*pred_cat),
        'LGB단독':                      (cv_lgb, pred_lgb),
        'Rank단순평균':                 (roc_auc_score(y_train, (r_lgb+r_xgb+r_cat)/3),                                 (rt_lgb+rt_xgb+rt_cat)/3),
        'Rank성능가중':                 (roc_auc_score(y_train, w_perf[0]*r_lgb+w_perf[1]*r_xgb+w_perf[2]*r_cat),      w_perf[0]*rt_lgb+w_perf[1]*rt_xgb+w_perf[2]*rt_cat),
        'Rank LGB강조(0.5/0.25/0.25)': (roc_auc_score(y_train, 0.5*r_lgb+0.25*r_xgb+0.25*r_cat),                      0.5*rt_lgb+0.25*rt_xgb+0.25*rt_cat),
        'Rank LGB+CAT(0.6/0.4)':       (roc_auc_score(y_train, 0.6*r_lgb+0.4*r_cat),                                   0.6*rt_lgb+0.4*rt_cat),
        'Rank LGB+CAT(0.5/0.5)':       (roc_auc_score(y_train, 0.5*r_lgb+0.5*r_cat),                                   0.5*rt_lgb+0.5*rt_cat),
    }
    ...
    for name, (auc, _) in sorted_results:
        print(f'    {auc:.5f}  {name}{"  ⭐ BEST" if name == best_name else ""}')

    print(f'\n  ─── 버전별 비교 ───')
    print(f'  v8plus_v3: 0.74076  (베이스)')
    print(f'  v8plus_v5: {best_cv:.5f}  {"🔺 UP" if best_cv > 0.74076 else "🔻 DOWN" if best_cv < 0.74076 else "➡️ 동일"}')

    os.makedirs(SAVE_PATH + 'oof/', exist_ok=True)
    for name, arr in [('oof_lgb', oof_lgb), ('oof_xgb', oof_xgb), ('oof_cat', oof_cat),
                      ('pred_lgb', pred_lgb), ('pred_xgb', pred_xgb), ('pred_cat', pred_cat)]:
        np.save(f'{SAVE_PATH}oof/{name}_{VERSION}.npy', arr)
    print(f'\n  💾 OOF 저장 완료!')
...
def print_feature_importance(importance):
    ...
    print(f'\n  📊 V5 신규 피처 중요도:')
    for f in ['총배아대비_해동비율', 'IVF_실패부담', '난자정자출처조합_te']:
        if f in importance['feature'].values:
            rank = importance['feature'].tolist().index(f) + 1
            imp  = importance[importance['feature'] == f]['importance'].values[0]
            print(f'    {f}: {imp:.1f} (전체 {rank}위)')
...
if __name__ == '__main__':
    ...
    X_train, y_train, X_test, feature_cols = build_v8_final_features(train, test)
