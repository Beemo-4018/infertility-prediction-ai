...
"""
====================================================
난임 환자 임신 성공 여부 예측 - v_clean3 (교호작용 피처 추가)
평가 지표  : ROC-AUC
...
CLEAN_FEATURES = [
    ...
    # H. 교호작용 피처 (상위 피처 조합, 클리닉 집계 후 생성)
    '배아이식비율_나이',    # 배아 품질 x 나이
    '클리닉_개인성공률',    # 클리닉 성공률 x 과거 임신 성공률
    '실패누적_나이',        # failure_streak x 나이
]
...
def preprocess(train, test):
    ...
    # ── H. 교호작용 피처 (클리닉 집계 완료 후 생성) ─────────────
    for df in [train, test]:
        df['배아이식비율_나이']  = df['배아_이식비율'] * df['나이_수치']
        df['클리닉_개인성공률'] = df['시술시기코드_성공률'] * df['과거_임신성공률']
    ...
    feature_desc = {
        ...
        '배아이식비율_나이'    : 'H. 배아이식비율 x 나이',
        '클리닉_개인성공률'    : 'H. 클리닉성공률 x 과거임신성공률',
        '실패누적_나이'        : 'H. failure_streak x 나이',
    }
...
if __name__ == '__main__':
    print('=' * 55)
    print('  난임 환자 임신 성공 여부 예측 (v_clean3)')
    print('  피처 수: 18개 (교호작용 피처 3개 추가)')
    print('=' * 55)
