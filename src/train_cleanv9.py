...
"""
====================================================
난임 환자 임신 성공 여부 예측 - v_clean9 (클리닉성공률 → 클리닉x나이 TE 대체)
평가 지표  : ROC-AUC
...
CLEAN_FEATURES = [
    # A. 환자 상태
    '나이_수치',           # 나이가 많을수록 성공률 하락 (나이_수치와 중복이라 고령_여부 제거)
    '과거_임신성공률',     # 과거 성공 경험

    # B. 이번 시술 정보
    '시술_ICSI',           # 미세주입 여부 (중요도 확인됨)
    '배란 자극 여부',      # 배란 유도 치료 여부
    # 제거: 시술_FER (중요도 0), 시술_BLASTOCYST (중요도 낮음)
    ...
    # H. 시간 간격 피처
    '혼합_이식_간격',      # 배양 기간 (중요도 11위 확인)
    '해동_이식_간격',      # 해동→이식 간격

    # I. 클리닉 x 나이 Target Encoding (시술시기코드_성공률 대체)
    '클리닉_나이별성공률', # 클리닉 x 나이그룹 조합 성공률 (더 세분화)
]
...
def preprocess(train, test):
    ...
    # ── I. 클리닉 x 나이 Target Encoding (K-Fold OOF) ────────────
    print('  클리닉 x 나이 Target Encoding 적용 중...')
    global_mean = train[TARGET].mean()
    skf_te      = StratifiedKFold(n_splits=5, shuffle=True, random_state=SEED)
    smooth      = 50

    train['클리닉_나이조합'] = (
        train['시술 시기 코드'].astype(str) + '_' + train['시술 당시 나이'].astype(str)
    )
    test['클리닉_나이조합'] = (
        test['시술 시기 코드'].astype(str) + '_' + test['시술 당시 나이'].astype(str)
    )
    col = '클리닉_나이조합'

    tr_enc = np.zeros(len(train))
    ...
    test['클리닉_나이별성공률']  = (
        test[col].map(full_smooth).fillna(global_mean).values
    )
    ...
    feature_desc = {
        ...
        '혼합_이식_간격'      : 'H. 혼합→이식 간격 (배양 기간)',
        '해동_이식_간격'      : 'H. 해동→이식 간격',
        '클리닉_나이별성공률' : 'I. 클리닉x나이 Target Encoding',
    }
...
if __name__ == '__main__':
    print('=' * 55)
    print('  난임 환자 임신 성공 여부 예측 (v_clean9)')
    print('  피처 수: 17개 (시술시기코드_성공률 제거 + 클리닉x나이 TE 대체)')
    print('=' * 55)
