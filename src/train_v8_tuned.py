...
"""
====================================================
난임 환자 임신 성공 여부 예측 - v8_tuned (XGB + CatBoost Optuna 추가)
평가 지표  : ROC-AUC
실행 방법  : python src/train_v8.py

[v8 변경사항 - v5 대비]
    1. 미사용 컬럼 활용
       - 난자/정자 기증자 나이 수치화
       - 동결/신선/기증 배아 사용 여부 명시적 플래그
       - 대리모 여부
       - 저장된 신선 난자 수
       - 해동 난자 수
       - 시술 유형 (IVF vs DI) Target Encoding
       - 미세주입 후 저장된 배아 수
    2. 기증 관련 교호작용 피처 추가
====================================================
"""
...
def preprocess(train, test):
    ...
    for df in [train, test]:
        ...
        # [v8 NEW] 기증 관련 교호작용
        df['기증난자_고령조합'] = (
            (df['난자기증자_있음'] == 1) & (df['고령_여부'] == 1)
        ).astype(int)
        df['기증난자_젊음_고령모'] = (
            (df['난자기증자_젊음'] == 1) & (df['고령_여부'] == 1)
        ).astype(int)
        # 기증 배아 사용 여부 (명시적)
...
# 4-2. Optuna XGBoost
# ====================================================
def optuna_xgb(X_train, y_train, n_trials=50):
    print('\n' + '=' * 55)
    print(f'[3-2] Optuna 튜닝 (XGBoost, {n_trials} trials)')
    print('=' * 55)
    ...
    def objective(trial):
        ...
        for tr_idx, val_idx in skf.split(X_train, y_train):
            ...
            y_tr, y_val = y_train.iloc[tr_idx], y_train.iloc[val_idx]
            m = xgb.XGBClassifier(
                **params, n_estimators=2000, early_stopping_rounds=50
            )
...
# 4-3. Optuna CatBoost
# ====================================================
def optuna_cat(X_train, y_train, n_trials=50):
    print('\n' + '=' * 55)
    print(f'[3-3] Optuna 튜닝 (CatBoost, {n_trials} trials)')
    print('=' * 55)
...
def train_xgb(X_train, y_train, X_test, best_params=None):
    print('\n' + '=' * 55)
    print('[4-2] XGBoost')
    print('=' * 55)
    if best_params:
...
def train_cat(X_train, y_train, X_test, best_params=None):
    print('\n' + '=' * 55)
    print('[4-3] CatBoost')
    print('=' * 55)
    if best_params:
...
if __name__ == '__main__':
    print('=' * 55)
    print('  난임 환자 임신 성공 여부 예측 (v8_tuned)')
    print('=' * 55)
    ...
    if USE_OPTUNA:
        lgb_best = optuna_lgb(X_train, y_train, n_trials=OPTUNA_TRIALS)
        xgb_best = optuna_xgb(X_train, y_train, n_trials=OPTUNA_TRIALS)
        cat_best = optuna_cat(X_train, y_train, n_trials=OPTUNA_TRIALS)

    lgb_result = train_lgb(X_train, y_train, X_test, lgb_best)
    xgb_result = train_xgb(X_train, y_train, X_test, xgb_best)
    cat_result = train_cat(X_train, y_train, X_test, cat_best)
