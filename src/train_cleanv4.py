...
"""
====================================================
난임 환자 임신 성공 여부 예측 - v_clean4 (교호작용 대체)
평가 지표  : ROC-AUC
...
CLEAN_FEATURES = [
    ...
    '이식된 배아 수',      # 이식 배아 수
    '미세주입_성공률',     # ICSI 성공률
    '배아이식비율_나이',   # 배아이식비율 x 나이 (원본 배아_이식비율 대체)
    ...
    '출산_경험',           # 출산 성공 경험
    '실패누적_나이',       # failure_streak x 나이 (원본 failure_streak 대체)

    # F. 클리닉
    '시술시기코드_시술건수', # 클리닉 규모
    '클리닉_개인성공률',   # 클리닉성공률 x 과거임신성공률 (두 원본 대체)
...
def preprocess(train, test):
    print('\n' + '=' * 55)
    print('[2] 전처리 & 핵심 피처 생성 (15개, 교호작용 대체)')
    print('=' * 55)
    ...
    # ── 교호작용 피처 생성 (클리닉 집계 완료 후) ────────────────
    for df in [train, test]:
        # 배아이식비율 x 나이 (원본 배아_이식비율 대체)
        df['배아이식비율_나이'] = df['배아_이식비율'] * df['나이_수치']

        # 클리닉성공률 x 과거임신성공률 (두 원본 대체)
        df['클리닉_개인성공률'] = (
            df['시술시기코드_성공률'] * df['과거_임신성공률']
        )

        # failure_streak x 나이 (원본 failure_streak 대체)
        df['실패누적_나이'] = df['failure_streak'] * df['나이_수치']
    ...
    feature_desc = {
        ...
        '배아이식비율_나이'    : 'C. 배아이식비율 x 나이 (대체)',
        '클리닉_개인성공률'    : 'F. 클리닉성공률 x 과거임신성공률 (대체)',
        '실패누적_나이'        : 'E. failure_streak x 나이 (대체)',
    }
...
if __name__ == '__main__':
    print('=' * 55)
    print('  난임 환자 임신 성공 여부 예측 (v_clean4)')
    print('  피처 수: 15개 (교호작용으로 원본 대체)')
    print('=' * 55)
