...
"""
====================================================
난임 환자 임신 성공 여부 예측 - v11 (TabNet + 트리 앙상블)
평가 지표  : ROC-AUC
실행 방법  : python src/train_v11.py

설치 필요:
    pip install pytorch-tabnet torch

[v11 변경사항]
    - TabNet 추가 (GPU 활용, 트리 모델과 다른 패턴 학습)
    - LightGBM + XGBoost + CatBoost + TabNet 4모델 앙상블
    - TabNet은 GPU 자동 감지 후 사용
...
USE_OPTUNA    = True
OPTUNA_TRIALS = 100   # GPU 있으니까 100으로 늘림
...
# ====================================================
# 3. 전처리 & 피처 엔지니어링 (v5 기반 + v10 시술 분해)
# ====================================================
def preprocess(train, test):
    print('\n' + '=' * 55)
    print('[2] 전처리 & 피처 엔지니어링 (v11)')
    print('=' * 55)
...
# ====================================================
# 4. Optuna (GPU 가속, 100 trials)
# ====================================================
def optuna_lgb(X_train, y_train, n_trials=100):
    print('\n' + '=' * 55)
...
def train_tabnet(X_train, y_train, X_test):
    print('\n' + '=' * 55)
    print(f'[4-4] TabNet (Stratified 5-Fold, device={DEVICE})')
    print('=' * 55)

    # TabNet은 StandardScaler 필요
    ...
    for fold, (tr_idx, val_idx) in enumerate(skf.split(X_train_sc, y_train)):
        ...
        y_tr  = y_train.iloc[tr_idx].values.reshape(-1, 1)
        ...
        pred         += m.predict_proba(X_test_sc)[:, 1] / N_FOLDS
        print(
            f'  Fold {fold+1}  AUC: {roc_auc_score(y_train.iloc[val_idx], val_prob):.5f}'
        )

    cv = roc_auc_score(y_train, oof)
    print(f'  TabNet CV AUC   : {cv:.5f}')
    return oof, pred, cv
...
def ensemble_and_save(y_train, sub, results):
    ...
    names = ['LightGBM', 'XGBoost', 'CatBoost', 'TabNet']
    ...
    # TabNet 제외 트리 앙상블도 별도 저장 (비교용)
    tree_results  = results[:3]
    tree_oofs     = [r[0] for r in tree_results]
    tree_preds    = [r[1] for r in tree_results]
    tree_cvs      = np.array([r[2] for r in tree_results])
    tree_weights  = tree_cvs / tree_cvs.sum()
    tree_oof      = sum(w * o for w, o in zip(tree_weights, tree_oofs))
    tree_pred     = sum(w * p for w, p in zip(tree_weights, tree_preds))
    tree_cv       = roc_auc_score(y_train, tree_oof)

    tree_file = f'{SAVE_PATH}submission_{timestamp}_tree_only_auc{str(tree_cv)[:7].replace(".", "p")}.csv'
    sub_tree  = sub.copy()
    sub_tree['probability'] = tree_pred
    sub_tree.to_csv(tree_file, index=False)
    print(f'  트리 전용    : {tree_file}')
    print(f'  트리 CV AUC  : {tree_cv:.5f}')
...
if __name__ == '__main__':
    print('=' * 55)
    print('  난임 환자 임신 성공 여부 예측 (v11 - TabNet + GPU)')
    print('=' * 55)
    ...
    tabnet_result = train_tabnet(X_train, y_train, X_test)

    ensemble_and_save(
        y_train, sub,
        [lgb_result, xgb_result, cat_result, tabnet_result]
    )