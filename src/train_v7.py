...
"""
====================================================
난임 환자 임신 성공 여부 예측 - v7
평가 지표  : ROC-AUC
실행 방법  : python src/train_v7.py

[v7 변경사항 - v5 대비]
    1. 나이 × 배아 수 상호작용 피처 추가
    2. 시술 횟수 구간 피처 추가
    3. 동결/신선 배아 × 나이 조합 피처 추가
    4. 클리닉 성공률 vs 개인 성공률 차이 피처 추가
    5. 배아 이식 수 구간 피처 추가 (Target Encoding)
    6. 스태킹 제거 → v5 단순 앙상블 구조 유지
====================================================
"""
...
def preprocess(train, test):
    ...
    for df in [train, test]:
        ...
        # ── [v7 NEW A] 나이 × 배아 수 상호작용 ──────────────
        df['나이_x_이식배아수']   = df['나이_수치'] * df['이식된 배아 수']
        df['나이_x_생성배아수']   = df['나이_수치'] * df['총 생성 배아 수']
        df['나이_x_배아이식비율'] = df['나이_수치'] * df['배아_이식비율']
        df['나이_x_난자수정률']   = df['나이_수치'] * df['난자_수정률']

        # ── [v7 NEW B] 시술 횟수 구간 ────────────────────────
        df['시술_구간'] = pd.cut(
            df['총 시술 횟수_num'].fillna(0),
            bins=[-1, 1, 3, 6, 99],
            labels=['첫시도', '초기', '중기', '장기']
        ).astype(str)

        # ── [v7 NEW C] 동결/신선 배아 × 나이 연속 조합 ───────
        df['동결배아_x_나이']  = df['동결배아_시술'] * df['나이_수치']
        df['신선배아_여부']    = (df['해동된 배아 수'] == 0).astype(int)
        df['신선배아_x_최적연령'] = df['신선배아_여부'] * df['최적연령_여부']
        df['신선배아_x_나이']  = df['신선배아_여부'] * df['나이_수치']

        # ── [v7 NEW E] 배아 이식 수 구간 (TE용 문자열) ────────
        df['이식배아_구간'] = pd.cut(
            df['이식된 배아 수'].fillna(0),
            bins=[-1, 1, 2, 3, 99],
            labels=['1개', '2개', '3개', '4개이상']
        ).astype(str)
    ...
    # ── [v7 NEW D] 클리닉 성공률 vs 개인 성공률 차이 ─────────
    # 클리닉 평균 대비 개인이 얼마나 어려운 케이스인지
    ...
    # ── Label Encoding ──────────────────────────────────────
    cat_cols = train[[c for c in train.columns if c not in drop_cols]]\
