...
"""
====================================================
난임 환자 임신 성공 여부 예측 - v_clean5 (핵심 10개 최소 베이스)
평가 지표  : ROC-AUC
...
# ====================================================
CLEAN_FEATURES = [
    # 중요도 순위 기반 핵심 10개만 선택
    # (v_clean 실험에서 중요도 상위 10개 확인됨)

    # 배아 품질 (1~2위)
    '배아_이식비율',       # 압도적 1위 - 배아 품질 핵심 지표
    '미세주입_성공률',     # 2위 - ICSI 성공률

    # 클리닉 (3~4위)
    '시술시기코드_시술건수', # 3위 - 클리닉 규모
    '시술시기코드_성공률', # 4위 - 클리닉 성공률

    # 환자 상태 (5~6위)
    '나이_수치',           # 5위 - 나이
    '과거_임신성공률',     # 6위 - 과거 임신 성공률

    # 배아 수 / 불임 원인 (7~8위)
    '이식된 배아 수',      # 7위 - 이식 배아 수
    '총_불임원인_수',      # 8위 - 불임 원인 복잡도

    # 과거 이력 (9~10위)
    '총 시술 횟수_num',    # 9위 - 총 시술 횟수
    'failure_streak',      # 10위 - 연속 실패 횟수
]
...
if __name__ == '__main__':
    print('=' * 55)
    print('  난임 환자 임신 성공 여부 예측 (v_clean5)')
    print('  피처 수: 10개 (핵심 피처만)')
    print('=' * 55)
