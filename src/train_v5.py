...
"""
====================================================
난임 환자 임신 성공 여부 예측 - v5 (클리닉 집계 피처 강화)
평가 지표  : ROC-AUC
실행 방법  : python train_v5.py
...
[v5 변경사항 - v3 대비]
    1. 클리닉 집계 피처 추가 (K-Fold OOF, leakage 없음)
       - 시술시기코드_성공률: 클리닉별 평균 임신 성공률
       - 시술시기코드_시술건수: 클리닉 규모 (로그 스케일)
       - 시술시기코드_나이별성공률: 클리닉 × 나이그룹 조합 성공률
       - 시술시기코드_시술유형별성공률: 클리닉 × 시술유형 조합 성공률
       - 시술시기코드_배아이식수평균: 클리닉별 평균 배아 이식 수
       - 시술시기코드_성공률편차: 클리닉 성공률 - 전체 평균 (클리닉 수준 측정)
...
def preprocess(train, test):
    ...
    # ── 3-6. 클리닉 집계 피처 (K-Fold OOF, leakage 없음) ─────
    # 클리닉(시술 시기 코드)별 집계 피처
    # train 내부 leakage 차단을 위해 K-Fold OOF 방식 사용
    print('  클리닉 집계 피처 생성 중...')
    clinic_col = '시술 시기 코드'
    global_mean = train[TARGET].mean()
    skf_clinic  = StratifiedKFold(n_splits=5, shuffle=True, random_state=SEED)

    # [1] 클리닉별 성공률 (smoothed target encoding)
    ...
    # [2] 클리닉 규모 (log 시술 건수) - 행 내 정보 아니므로 OOF 방식
    ...
    # ── 3-6b. Target Encoding (K-Fold OOF, leakage 없음) ───────
    # 단일 카테고리 컬럼
