...
"""
====================================================
난임 환자 임신 성공 여부 예측 - v10 (시술 유형 분해 + 미사용 컬럼 정제)
평가 지표  : ROC-AUC
실행 방법  : python src/train_v10.py

[v10 변경사항 - v5 대비]
    1. 특정 시술 유형 분해 (핵심 변경)
       - ICSI, FER, BLASTOCYST, AH, IUI, GIFT 각각 이진 플래그
       - 복합 시술 여부, 시술 개수
       - FER × 동결배아 교호작용 (동결 배아 이식 매칭)
       - BLASTOCYST × 혼합_이식_간격 교호작용
    2. 단일 배아 이식 여부 활용 (미사용 컬럼)
       - 단일 배아 이식 × 나이 교호작용
       - 단일 배아 이식 × 배아 품질 교호작용
    3. 시술 유형(IVF/DI) 명시적 활용
       - 지금까지 특정 시술 유형만 썼고 상위 분류인 시술 유형을 안 썼음
    4. 기증자 정자와 혼합된 난자 수 비율 (정제해서 재활용)
       - v8에서 노이즈였던 기증자 나이 제거, 수치 비율만 사용
...
def preprocess(train, test):
    ...
    for df in [train, test]:
        ...
        # ── [v10 NEW 1] 특정 시술 유형 분해 ──────────────────────
        # "ICSI/BLASTOCYST", "ICSI:IVF" 같은 복합 문자열을 개별 플래그로 분해
        ...
        df['시술_GIFT']        = 시술유형.str.contains('GIFT',       na=False).astype(int)
        df['시술_복합여부']    = 시술유형.str.contains(r'[/:]',      na=False).astype(int)
        df['시술_개수']        = (
            시술유형.str.count(r'[/:]') + 1
        ).where(시술유형 != 'nan', 1)

        # FER(동결 배아 이식) × 동결배아 시술 매칭
        df['FER_동결배아매칭'] = (
            (df['시술_FER'] == 1) & (df['동결배아_시술'] == 1)
        ).astype(int)

        # BLASTOCYST × 혼합→이식 간격 (배반포 이식 확인)
        df['BLASTOCYST_간격매칭'] = (
            (df['시술_BLASTOCYST'] == 1) & (df['혼합_이식_간격'] >= 5)
        ).astype(int)

        # ICSI × 남성 불임 원인 매칭 (기존보다 더 세밀한 버전)
        df['ICSI_정자문제매칭'] = (
            (df['시술_ICSI'] == 1) & (sperm_issues > 0)
        ).astype(int)

        # ── [v10 NEW 2] 단일 배아 이식 여부 활용 ─────────────────
        # 단일 배아 이식은 의학적으로 성공률과 직결
        df['단일배아_고령조합'] = (
            (df['단일 배아 이식 여부'] == 1) & (df['고령_여부'] == 1)
        ).astype(int)
        df['단일배아_초보조합'] = (
            (df['단일 배아 이식 여부'] == 1) & (df['총 시술 횟수_num'] <= 1)
        ).astype(int)

        # ── [v10 NEW 3] 시술 유형 (IVF/DI) 명시적 피처 ──────────
        df['IVF시술_여부'] = (df['시술 유형'] == 'IVF').astype(int)

        # ── [v10 NEW 4] 기증자 정자 비율 (나이 제외, 수치만) ──────
        df['기증정자_비율'] = (
            df['기증자 정자와 혼합된 난자 수'] / (df['혼합된 난자 수'] + 1e-6)
        )
        df['기증정자_사용'] = (df['기증자 정자와 혼합된 난자 수'] > 0).astype(int)

        # ── [v10 NEW 5] 해동 난자 수 활용 ────────────────────────
        df['해동난자_비율'] = df['해동 난자 수'] / (df['수집된 신선 난자 수'] + 1e-6)
        df['해동난자_사용'] = (df['해동 난자 수'] > 0).astype(int)

        # ── [v10 NEW 6] 저장된 신선 난자 수 활용 ─────────────────
        df['신선난자_저장비율'] = (
